...
(function ($) {
	...
	////////////////////////////////////////////////////
	// 03.1 Set active link in main navigation
	function setActiveNavLink() {
		// Get current page URL
		var currentPage = window.location.pathname.split('/').pop();
		
		// If we're on the home page (no file name), default to index.html
		if (currentPage === '' || currentPage === '/') {
			currentPage = 'index.html';
		}
		
		// Make sure the navigation elements exist before trying to manipulate them
		if ($('.tp-main-menu.tp-main-menu-md ul li').length > 0) {
			// Remove active class from all navigation links in the main menu
			// This targets both regular and sticky header navigation
			$('.tp-main-menu.tp-main-menu-md ul li a').removeClass('active');
			
			// Add active class based on current page
			if (currentPage === 'index.html') {
				// For home page, add active class to the Home link (first link)
				$('.tp-main-menu.tp-main-menu-md ul li:first-child a').addClass('active');
			} else if (currentPage === 'about.html') {
				// For about page, add active class to the About Us link (second link)
				$('.tp-main-menu.tp-main-menu-md ul li').eq(1).find('a').addClass('active');
			} else if (currentPage === 'researchers.html') {
				// For researchers page, add active class to the For Researchers link (third link)
				$('.tp-main-menu.tp-main-menu-md ul li').eq(2).find('a').addClass('active');
			} else if (currentPage === 'industry.html') {
				// For industry page, add active class to the For Industry link (fourth link)
				$('.tp-main-menu.tp-main-menu-md ul li').eq(3).find('a').addClass('active');
			} else if (currentPage === 'services.html') {
				// For services page, add active class to the Services link (fifth link)
				$('.tp-main-menu.tp-main-menu-md ul li').eq(4).find('a').addClass('active');
			} else if (currentPage === 'contact.html') {
				// For contact page, add active class to the Contact link (sixth link)
				$('.tp-main-menu.tp-main-menu-md ul li').eq(5).find('a').addClass('active');
			}
		}
	}
	...
	$(document).ready(function() {
		// Call it immediately
		setActiveNavLink();
		// Also call it after a small delay to ensure all elements are loaded
		setTimeout(setActiveNavLink, 100);
		// Call it again after a longer delay to ensure it overrides any hardcoded active classes
		setTimeout(setActiveNavLink, 500);
		
		// Also call it when the header becomes sticky
		$(window).on('scroll', function() {
			setTimeout(setActiveNavLink, 50);
		});
	});
	
	// Reapply active link styling when ScrollTrigger updates (to override its removal of active classes)
	...
	$(".tp-search-btn").on("click", function () {
		$(".tp-search-area").addClass("opened");
		$(".body-overlay").addClass("opened");
	});

	$(".cartmini-btn").on("click", function () {
	...
	// Close offcanvas when window is resized to 1199px or larger
	...
	var smoother = ScrollSmoother.create({
		...
		onUpdate: function(self) {
			// Update any custom scroll-dependent animations here if needed
		}
	});
	
	// Handle anchor link clicks with improved error handling for medical theme
	$(document).on('click', 'a[href^="#"]:not([data-bs-toggle="tab"])', function(e) {
		var target = $(this).attr('href');
		var $target = $(target);
		
		// Check if target exists and is not just "#"
		if(target !== '#' && $target.length) {
			e.preventDefault();
			
			// Add active class to clicked link for medical theme styling
			$('a[href^="#"]:not([data-bs-toggle="tab"])').removeClass('active');
			...
			try {
				...
				// Fallback to regular scroll animation if ScrollSmoother fails
	...
	// Also handle the case where anchor links might be added dynamically
	$(window).on('load', function() {
		// Reattach event handlers if needed
		$('a[href^="#"]:not([data-bs-toggle="tab"])').off('click').on('click', function(e) {
			var target = $(this).attr('href');
			var $target = $(target);
	...
	// Handle smooth scrolling for same-page links with data-scroll attribute
	$(document).on('click', 'a[data-scroll]:not([data-bs-toggle="tab"])', function(e) {
		var target = $(this).attr('href');
		if (target && target.charAt(0) === '#') {
			...
			if ($target.length) {
				...
				// Add active class to clicked link for medical theme styling
				$('a[data-scroll]:not([data-bs-toggle="tab"])').removeClass('active');
				$(this).addClass('active');
	...
	// Update active navigation link based on scroll position for medical theme
	// Modified to not interfere with page-based navigation active links
	ScrollTrigger.create({
		start: 0,
		end: "max",
		onUpdate: function(self) {
			var scrollPos = self.scroll();
			var sections = ['hero', 'features', 'about', 'services', 'team', 'blog', 'contact'];
			
			// Only remove active class from anchor links, not page links
			$('a[href^="#"]:not([data-bs-toggle="tab"])').removeClass('active');
			
			// Find the current section and add active class to corresponding nav link
			for (var i = sections.length - 1; i >= 0; i--) {
				var section = $('#' + sections[i]);
				if (section.length && scrollPos >= section.offset().top - 100) {
					$('a[href="#' + sections[i] + '"]:not([data-bs-toggle="tab"])').addClass('active');
					break;
				}
			}
			
			// Reapply page-based active link after a short delay
			setTimeout(function() {
				setActiveNavLink();
			}, 10);
		}
	...
	// Video popup for both YouTube and local videos
	$('.popup-video').magnificPopup({
		...
		iframe: {
			...
			patterns: {
				...
				local: {
					index: function(src) {
						if (src && (src.includes('.mp4') || src.includes('.webm') || src.includes('.ogg'))) {
							return src;
						}
						return false;
					},
		...
		callbacks: {
			elementParse: function(item) {
				...
				if (src && (src.includes('.mp4') || src.includes('.webm') || src.includes('.ogg'))) {
					// Local video file - create a simple HTML page with the video
					var htmlContent = '<!DOCTYPE html><html><head><style>body{margin:0;padding:0;background:#000;display:flex;justify-content:center;align-items:center;height:100vh;}video{max-width:100%;max-height:100%;width:auto;height:auto;}</style></head><body><video controls autoplay><source src="' + src + '" type="video/mp4">Your browser does not support the video tag.</video></body></html>';
					item.src = 'data:text/html;charset=utf-8,' + encodeURIComponent(htmlContent);
				} else {
